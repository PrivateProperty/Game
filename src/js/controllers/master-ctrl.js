...
function MapCtrl($scope, gameService) {
    ...
    $scope.cityComputer = [];
    $scope.cityFromUser = [];
    $scope.cityAll = ["Москва", "Минск"];
    //console.log($scope);


    $scope.game = function (fromUser) {
        $scope.cityFromUser.push(fromUser);
        $scope.cityAll.push(fromUser);
    ...
    $scope.checkCity = function (cityX) {
        if ($scope.isCyrillic(cityX)) {
            if ($scope.cityAll.indexOf(cityX) > 0) {
    ...
    $scope.compucterStep = function (fromUser) {
        ...
        for (var i = 0; i < $scope.cityComputerAll.length; i++) {

            if (x === $scope.cityComputerAll[i].slice(0, 1).toLowerCase()) {
                console.log("Есть");
                $scope.cityComputer.push($scope.cityComputerAll[i]);
                $scope.cityAll.push($scope.cityComputerAll[i]);
                $scope.cityComputerAll.splice(i, 1);
                break
            } else if (!$scope.cityComputerAll[i + 1]) {
